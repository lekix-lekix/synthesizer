...
private slots:
    ...
    void                    onDisconnectionRequest(float *from, float *to) {
        std::vector<std::unique_ptr<Patch>> &patches = synth_.getConnections();
        int idx = 0;

        std::cout << "onDisconnection" << std::endl;
        for (auto &patch : patches) {
                if (patch->getFrom() == from && patch->getTo() == to) {
                    synth_.deleteConnection(idx);
                    return ;
            }
            idx++;
        }
